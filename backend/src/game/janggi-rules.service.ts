...
@Injectable()
export class JanggiRulesService {
  /**
   * 기물의 이동 가능한 모든 위치를 반환 (규칙만 고려, 장군 상태 미고려)
   */
  getPossibleMoves(piece: PieceData, board: PieceData[]): Position[] {
    switch (piece.type) {
      case '마':
        return this.knightMoves(piece, board);
      case '차':
        return this.chaMoves(piece, board);
      case '상':
        return this.elephantMoves(piece, board);
      case '사':
   ...
   * 특정 팀의 왕이 장군 상태인지 확인
   */
  isCheck(board: PieceData[], team: 'cho' | 'han'): boolean {
    // 현재 team의 왕 찾기
   ...
   * 마 이동 규칙
   */
  private knightMoves(piece: PieceData, board: PieceData[]): Position[] {
    const moves: Position[] = [];
    const { x, y } = piece;
   ...
   * 상 이동 규칙
   */
  private elephantMoves(piece: PieceData, board: PieceData[]): Position[] {
    const moves: Position[] = [];
    const { x, y, team } = piece;
   ...
   * 차 이동 규칙
   */
  private chaMoves(piece: PieceData, board: PieceData[]): Position[] {
    const moves: Position[] = [];
    const { x, y, team } = piece;
   ...
   * 포 이동 규칙
   */
  private poMoves(piece: PieceData, board: PieceData[]): Position[] {
    const moves: Position[] = [];
    const { x, y, team } = piece;
   ...
   * 왕/사 이동 규칙
   */
  private kingMoves(piece: PieceData, board: PieceData[]): Position[] {
    const moves: Position[] = [];
    const { x, y, team } = piece;
